...
export default function Navbar() {
  ...
  const [csharpPosts, setcsharpPosts] = useState(null);
  const [reactPosts, setReactPosts] = useState(null);
  ...
  useEffect(() => {
    sanityClient
      ...
      .then((data) => setJavascriptPosts(data))
      .catch(console.error);
  }, []);
      ...
      <div className="sticky top-0 ">
        ...
        <ul className="list-unstyled d-md-block d-flex nav-scroller ">
          <li className="mx-2 my-1">
            <button
              className="btn btn-toggle align-items-center rounded collapsed fs-5 border-0"
              data-bs-toggle="collapse"
              data-bs-target="#variables-javascript"
              aria-expanded="false"
            >
              <DiJavascript1 className="text-warning me-1 fs-3" /> Články
            </button>
            <div className="collapse" id="variables-javascript">
          ...
          <li className="mx-2 my-1">
            <button
              className="btn btn-toggle align-items-center rounded collapsed fs-5 "
              data-bs-toggle="collapse"
              data-bs-target="#variables-reactJs"
              aria-expanded="false"
            >
              <DiReact className="text-info me-1 fs-3" /> Články
            </button>
            <div className="collapse" id="variables-reactJs">
          ...
          <li className="mx-2 my-1">
            <button
              className="btn btn-toggle align-items-center rounded collapsed fs-5 "
              data-bs-toggle="collapse"
              data-bs-target="#variables-csharp"
              aria-expanded="false"
            >
              <TbCSharp className="text-primary me-1 ms-1  fs-3" /> Články
            </button>
            <div className="collapse" id="variables-csharp">
              <ul className="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                {csharpPosts &&
                  [...csharpPosts]
                    .sort((a, b) => a.ids - b.ids)
