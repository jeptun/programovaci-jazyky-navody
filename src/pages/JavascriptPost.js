...
export default function JavascriptPosts() {
  ...
  useEffect(() => {
    sanityClient
      ...
      .then((newsData) => setNewsPost(newsData[newsData.length - 1]))
      .catch(console.error);
  }, []);
        ...
        <div>
          <section className="py-6 sm:py-12 ">
            <div className="container p-6 mx-auto space-y-8">
              ...
              <Link
                to={"/post/" + newsPost.slug.current}
                key={newsPost.slug.current}
                className="block max-w-sm gap-3 mx-auto sm:max-w-full group hover:no-underline focus:no-underline lg:grid lg:grid-cols-12 rounded-lg shadow-md  border"
              >
                <img
                  src={newsPost.mainImage.asset.url}
                  alt={newsPost.title}
                  className="object-cover w-full h-64 rounded sm:h-96 lg:col-span-7  rounded-tl-lg rounded-bl-lg"
                />
